...
| o calendário do IPVA e do licenciamento pelo final da placa (a tabela por estado e ano, com fonte) | `lib/app/calendarioDaPlaca.ts` (puro; conferido em `npm run conferir:datas`); a folha em `components/app/DatasDoCarro.tsx`. Para incluir um estado: uma entrada em `CALENDARIOS` com os dez finais, o ano e a URL da fonte |
