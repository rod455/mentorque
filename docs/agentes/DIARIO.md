...
| Quantos carros cadastrados existem? | **42**, em 22/09, contando os que estão na nuvem (`user_state`) de contas com login: 39 contas, 2 com mais de uma, maior frota 3, e ZERO repetidos dentro da mesma conta. Duas armadilhas: `estado_da_base` responde CONTAS COM CARRO (39), não carros; e o evento `cadastrou_carro` dá 67 APARELHOS, que é armazenamento de navegador e infla a cada instalação, então é teto inflado e não contagem. Carro de convidado vive no aparelho e nunca chega ao banco. | 22/09 |
