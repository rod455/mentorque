...
| 2026-09-07 | No Google Cloud, criar um cliente OAuth do tipo **Android** no mesmo projeto do cliente Web, com o pacote `mentorque.app` e os DOIS SHA-1: o da chave de upload (keystore do Codemagic) e o do Play App Signing (Play Console → App integrity) | O plugin da folha nativa já está no binário do Android (1.9), mas o Google só abre a caixinha se reconhecer pacote + assinatura; sem isso recusa com "Developer console is not set up correctly" e o app cai no navegador | Esta rodada |
| 2026-09-07 | Se a tela de consentimento OAuth do projeto estiver em "Testing", adicionar em Audience → Test users as contas Google que vão testar (a sua e a da Luana) | Em modo Testing o Google só deixa entrar quem está na lista; a folha abre e falha para todo o resto | Esta rodada |
| 2026-09-07 | Pôr o `NEXT_PUBLIC_GOOGLE_WEB_CLIENT_ID` em "Authorized Client IDs" no provider Google do Supabase | Sem isso o Supabase recusa o idToken da folha nativa com "invalid audience", e o login nativo do Android e do iPhone morre no último passo | Esta rodada |
