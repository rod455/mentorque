...
| 2026-09-07 | Ao publicar a 2.1, acrescentar `"2.1"` à lista `JA_PUBLICADAS` em `scripts/verifica-versoes.mjs` | Foi esquecer isto na 1.8 que deixou a conferência calada e mandou um segundo build com o mesmo nome de versão, cegando o funil e a tabela de erros para a diferença entre os dois | Esta rodada |
...
| 2026-09-09 | No App Store Connect, criar a versão 2.0, anexar o build que o Codemagic subiu, colar as notas de `docs/lojas/novidades-2.0.md` e enviar para revisão | A App Store está na 1.7 desde 03/09 (coletor do Analista, 09/09): nada do que subiu desde então chegou ao iPhone, e subir build no Codemagic não cria versão na loja, só a coloca no TestFlight | Esta rodada |
