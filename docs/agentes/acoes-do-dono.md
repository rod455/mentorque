...
| 2026-09-22 | Rodar `curl -sIL https://mentorque.com.br/barulho-no-carro` e mandar as linhas de HTTP e de location. Esperado: um 308 cujo `location` MANTÉM o caminho, e depois 200. Se vier três saltos, ou o location jogar para a raiz, ou voltar para a mesma URL, é erro de verdade e o conserto é de domínio na Vercel. O outro arquivo ("Página com redirecionamento", as duas URLs do apex) é informativo e nunca vai validar: pode ignorar para sempre | A exportação chegou em 22/09 e é UMA URL só, com status Pendente e último rastreamento em 01/09, ou seja o Google nem reconferiu. A página existe no repositório, o sitemap e o canonical usam www, e não há link absoluto sem www em nada que vire HTML. Falta a única coisa que não dá para deduzir do código: o que o servidor responde. O proxy desta sessão recusa mentorque.com.br | Esta rodada |
