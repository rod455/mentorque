...
| 2026-09-12 | Ligar a jornada de e-mails: ver o ensaio (`GET /api/cron/jornada?chave=`), receber as cópias de prova (`POST` com `teste`), e então `JORNADA_ATIVA=sim` na Vercel com redeploy. Roteiro em `docs/jornada.md` | A jornada está construída e conferida, e não manda nada para ninguém até esta chave existir. Cada dia sem ela é um dia sem e-mail para quem sumiu | Engenharia |
