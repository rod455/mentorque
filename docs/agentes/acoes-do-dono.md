...
| 2026-09-07 | Decidir se o `@capgo/capacitor-social-login` entra no binário do Android (com `providers: { facebook: false }`), o que muda o build das DUAS lojas e pede teste em aparelho | Sem ele no binário, a `NEXT_PUBLIC_GOOGLE_WEB_CLIENT_ID` no build do Android não liga folha nativa nenhuma: o app cai para o navegador, que é o caminho que em quatro semanas não produziu uma conta | Esta rodada |
