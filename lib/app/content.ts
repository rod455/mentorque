...
export function getContent(locale: Locale) {
  ...
  return {
    ...
    symptomsUi: {
      ...
      testesTitle: T("Confira você mesmo, antes de ir", "Check it yourself, before you go"),
      testesSub: T("Leva minutos, não precisa de ferramenta e já estreita o problema.", "Takes minutes, needs no tools, and narrows the problem down."),
      testesEntao: T("O que isso quer dizer", "What that means"),
    ...
    combustivel: {
      cardTitulo: T("Custo do carro", "Car cost"),
      cardSemana: T("Esta semana: {valor}", "This week: {valor}"),
      cardPorKm: T("{valor} por km", "{valor} per km"),
      cardFaltaUm: T("Mais um abastecimento e o custo por km aparece", "One more fill-up and cost per km shows up"),
      cardVazioTitulo: T("Quanto o {carro} custa por km?", "How much does the {carro} cost per km?"),
      cardVazioSub: T("Registre o próximo abastecimento em três toques. O km entra no calendário de graça.", "Log the next fill-up in three taps. The mileage feeds the calendar for free."),
      cardCta: T("Abasteci", "I filled up"),
      cardCtaVazio: T("Registrar abastecimento", "Log a fill-up"),
      titulo: T("Abastecimento", "Fill-up"),
      intro: T("Valor, litros e o km do painel. Só isso.", "Amount, liters and the dash mileage. That's it."),
      valor: T("Valor pago (R$)", "Amount paid (R$)"),
      valorPh: T("ex.: 180", "e.g. 180"),
      litros: T("Litros (opcional)", "Liters (optional)"),
      litrosPh: T("ex.: 32,5", "e.g. 32.5"),
      km: T("Km do painel", "Dash mileage"),
      kmPh: T("último: {km} km", "last: {km} km"),
      data: T("Data", "Date"),
      combustivel: T("Combustível", "Fuel"),
      tipos: { gasolina: T("Gasolina", "Gasoline"), etanol: T("Etanol", "Ethanol"), diesel: T("Diesel", "Diesel"), gnv: T("GNV", "CNG"), outro: T("Outro", "Other") },
      salvar: T("Salvar abastecimento", "Save fill-up"),
      kmMenor: T("O km informado ({novo}) é menor que o registrado ({atual}). O km só anda para frente; confira o painel.", "The mileage entered ({novo}) is lower than the recorded one ({atual}). Mileage only goes up; check the dash."),
      valorInvalido: T("Digite o valor pago, em reais.", "Type the amount paid."),
      kmInvalido: T("Digite o km do painel, em números.", "Type the dash mileage in numbers."),
      devolucaoTitulo: T("Registrado. O {carro} hoje:", "Saved. Your {carro} today:"),
      porKm: T("por km", "per km"),
      consumo: T("{n} km por litro", "{n} km per liter"),
      mes: T("Este mês em combustível: {valor}", "Fuel this month: {valor}"),
      primeiro: T("Primeiro abastecimento guardado. O próximo destrava o custo por km e o consumo.", "First fill-up saved. The next one unlocks cost per km and consumption."),
      kmCarimbado: T("O km do calendário foi atualizado para {km}.", "The calendar mileage was updated to {km}."),
      pronto: T("Pronto", "Done"),
      verHistorico: T("Ver no histórico", "See in history"),
      apagar: T("Apagar este abastecimento", "Delete this fill-up"),
      apagado: T("Abastecimento apagado.", "Fill-up deleted."),
      naoEncontrado: T("Esse abastecimento não está mais aqui.", "That fill-up is no longer here."),
      linha: T("Abastecimento", "Fill-up"),
      linhaLitros: T("{n} L", "{n} L"),
      faixaMes: T("Este mês: combustível {combustivel} · serviços {servicos}", "This month: fuel {combustivel} · services {servicos}"),
      semCarro: T("Cadastre o carro para registrar abastecimentos.", "Add the car to log fill-ups."),
    },
