...
export function PhoneFrame({ children }: { children: ReactNode }) {
  return (
    <div className="min-h-screen w-full overflow-x-hidden bg-graphite-900 text-cream antialiased">
      <div className="mx-auto flex min-h-screen w-full max-w-[440px] flex-col bg-graphite pb-[env(safe-area-inset-bottom)] pt-[env(safe-area-inset-top)] shadow-card sm:my-0 sm:min-h-screen">
        {children}
      </div>
    </div>
  );
...
export function Sheet({ open, onClose, children }: { open: boolean; onClose: () => void; children: ReactNode }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end justify-center" role="dialog" aria-modal="true">
      ...
      <div className="relative w-full max-w-[440px] animate-fade-up rounded-t-3xl bg-graphite-800 p-5 pb-[calc(1.75rem+env(safe-area-inset-bottom))] ring-1 ring-white/10">
        <button
          onClick={onClose}
